  		...
  		alert(arr);
	   	if(flag == true){
	   	    if(confirm("您确定要删除就这些记录吗?"))
	   		    location.href="../deleteCheckTicketpurchaseServlet?arr="+arr;
	   	}else
 ...
 // 		alert(arr);
	   	if(flag == true){
	   	    if(confirm("您确定要删除就这条记录吗?"))
	   		    location.href="../deleteTicketpurchaseServlet?arr="+arr;
	   	}else
...
<!-- Begin page -->
		...
		<div class="content-page">
			...
			<div class="content">
				<div class="card-box">
				<p style="margin-bottom:20px;margin-left:20px"><font size=4px>所有机票订单</font></p>
					<!-- Row start -->
					<div class="am-g">
		...
		<div class="am-u-sm-12">
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main">
              <thead>
	              <tr>
	                <th></th><th class="table-title">用户ID</th><th class="table-title">机票ID</th><th class="table-type">购买时间</th><th class="table-set">状态</th><th class="table-set">操作</th>
	              </tr>
              </thead>
	              <tbody>
			            <%
				            Vector<TicketpurchaseDTO> v = new Vector<TicketpurchaseDTO>();
				            v = (Vector<TicketpurchaseDTO>)session.getAttribute("allTicketpurchase");//这里的session与servlet的session一样				            
				            ...
				            while(it.hasNext()){
				            	...
				            	if(c.getStatus()!=1){
			           %>
				            <td><input type="checkbox" name="check" value="<%=c.getCid()%>,<%=c.getTid()%>"></td>
				            <td class="am-hide-sm-only"><%=c.getCid()%></td>
				            <td class="am-hide-sm-only"><%=c.getTid()%></td>
				            <td class="am-hide-sm-only"><%=c.getPurchaseDate()%></td>
				            <td class="am-hide-sm-only"><%=c.getStatus()%></td>
				         <!--  
				           <td>
				              <a href="updateTicketpurchaseInfo.jsp?cid=<%=c.getCid()%>&&tid=<%=c.getTid()%>">修改</a>
				              <a href="../deleteTicketpurchaseServlet?cid=<%=c.getCid()%>&&tid=<%=c.getTid()%>">删除</a>
				            </td>		
				         -->	           
				            <td>
				                  <div class="am-btn-toolbar">
				                    <div class="am-btn-group am-btn-group-xs">
				                      <button onClick="window.open('updateTicketpurchaseInfo.jsp?cid=<%=c.getCid()%>&&tid=<%=c.getTid()%>')" class="am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span>修改</button>
				                      <button class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" onclick="deleteCheck();"><span class="am-icon-trash-o"></span>删除</button>
				                    </div>
				                  </div>	                  
				            </td>
				        <tr>
 	      ...
 	      <%
 	        String flag="false";
 	        while(flag == "flase"){
	           flag = (String)session.getAttribute("allowance");	        
	      %>
	      <script type="text/javascript">alert("您要购买的机票已售完");</script>	      
	      <% flag = "true";}  %> 
            <!-- navbar -->
